...
try {
    $repo = new RuleRepository($pdo); $setRepo=new RuleSetRepository($pdo); $sets=$setRepo->all(); $selectedId=filter_input(INPUT_GET,'rule_set_id',FILTER_VALIDATE_INT)?:($setRepo->active()['id']??0); $selected=$setRepo->find((int)$selectedId);
    $parameters = $repo->parameters((int)$selectedId);
...
<section class="panel"><div class="panel-head"><div><h2>Parameter usage — Rule Set v<?= (int)$selected['version'] ?></h2><form method="get"><select name="rule_set_id" onchange="this.form.submit()"><?php foreach($sets as $set):?><option value="<?= (int)$set['id'] ?>" <?= $set['id']==$selectedId?'selected':'' ?>>v<?= (int)$set['version'] ?> <?= e(str_replace('_',' ',$set['status'])) ?></option><?php endforeach;?></select></form><p>All input parameters referenced by active and inactive rules.</p></div></div>
