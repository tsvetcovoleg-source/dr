...
$rule=$repo->find($id); if(!$rule){http_response_code(404);exit('Rule not found');} try { if(($_POST['action']??'')==='delete'){ $repo->delete($id); $_SESSION['flash']='Rule deleted.'; }
elseif(($_POST['action']??'')==='toggle'){ $repo->setActive($id,($_POST['active']??'0')==='1'); $_SESSION['flash']='Rule status updated.'; }
else {http_response_code(400);exit('Invalid action');} $user=$auth->requireLogin();$event=($_POST['action']??'')==='delete'?'RULE_REMOVED':(($_POST['active']??'0')==='1'?'RULE_ACTIVATED':'RULE_DEACTIVATED');$version=(int)$pdo->query('SELECT version FROM rule_sets WHERE id='.(int)$rule['rule_set_id'])->fetchColumn();$auth->audit()->log($event,(int)$user['id'],$user['username'],'rule',$id,['rule_set_id'=>(int)$rule['rule_set_id'],'rule_set_version'=>$version,'rule_code'=>$rule['rule_code']]); } catch(DomainException $e){http_response_code(403);exit($e->getMessage());} redirect('rules.php');
