...
final class RuleSetRepository
{
    ...
    public function createDraftFromActive(int $userId, ?string $name, ?string $comment): int
    {
        $this->pdo->beginTransaction();try {
            ...
            $ir=$this->pdo->prepare('INSERT INTO rules(rule_set_id,rule_code,stage_name,avg_actual_pd,priority,active,description) VALUES(?,?,?,?,?,?,?)');$cs=$this->pdo->prepare('SELECT field_name,operator,value,sort_order FROM rule_conditions WHERE rule_id=? ORDER BY sort_order,id');$ic=$this->pdo->prepare('INSERT INTO rule_conditions(rule_id,field_name,operator,value,sort_order) VALUES(?,?,?,?,?)');
            foreach($rules->fetchAll() as $r){$ir->execute([$draftId,$r['rule_code'],$r['stage_name'],$r['avg_actual_pd'],$r['priority'],$r['active'],$r['description']]);$new=(int)$this->pdo->lastInsertId();$cs->execute([(int)$r['id']]);foreach($cs->fetchAll() as $c)$ic->execute([$new,$c['field_name'],$c['operator'],$c['value'],$c['sort_order']]);}
